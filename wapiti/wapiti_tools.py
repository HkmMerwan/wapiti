...
def find_optimal_rejection(D, time_binned, rvs_binned, drvs_binned, frequency, n_components):
    """
    ...
    while D[index_sort][d_idx] >= 5:
        ...
        for idx in tqdm(range(len(time_used)), leave=False):
            rv_temp, std_rv_temp = odd_ratio_mean(rvs_used[idx], drvs_used[idx])
