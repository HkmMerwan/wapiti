...
def permute_and_fit_pca(X, weights, n, n_components, regularization):
    ...
    pca.regularization = regularization  # Fix regularization at 2
    pca.fit(X_sim, weights=weights_sim)
    return pca


def compute_variance_ratios(X, weights, n_components, N_permutations, regularization):
    ...
    for i in tqdm(range(N_permutations), leave=False):
        pca = permute_and_fit_pca(X, weights, n, n_components, regularization = regularization)
        variance[i] = pca.explained_variance_ratio_
    return variance


def compute_p_values(X, weights, N_permutations=100, regularization=0):
    ...
    pca_0.regularization = regularization  # Fix regularization at 2
    pca_0.fit(X, weights=weights)
    
    # Compute explained variance ratios for permuted models
    variance = compute_variance_ratios(X, weights, n_components, N_permutations, regularization)
...
def lpocv(X, weights, n_components, p=.8, N=100, regularization=0):
    """
